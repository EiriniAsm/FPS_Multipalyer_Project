...
public class FPSController : NetworkBehaviour {
	...
	void Start () {
		...
		//You can  locate a specific child object by name in the Scene Hierarchy using the Transform.Find method: transform.Find("Obj Name"); 
		//GameObject.Find("Obj Name"); locates GameObjects by name anywhere in the Scene Hierarchy 
		charController = GetComponent<CharacterController> ();
		speed = walkSpeed;
		is_Moving = false;

		rayDistance = charController.height * 0.5f + charController.radius;//we'll use this to check if we are on the ground when we're crouching
	...
	void PlayerMovement () {
		...
		//We use this lines of code to be sure that when the player moves diagonically, the speed doesnot exceed the speed we have set to be.
		...
		//inputY_Set != 0 && inputX_Set != 0 && limitDiagonalSpeed) ? 0.75f : 1.0f this is called trenary(triadikos) operator
		//condition ? expressionResultForTrue : expressionResultForFalse, we use a ternary operator instead of an if statement,
		//if our code needs a simple if()...else structure with a single short expression in each case.
		...
		//we use localEulerAngles because FP_view is a child to the "FPS Player" parent. So, if we need to change its transform we need to change it locally.
		...
		if (is_Grounded) {
			...
			//in order to move the player we need to convert the coordinates moveDirection from local (in relation to the parent's axis) to world space
